        ...
        if ($trk_news_active && $trk_permission != 2) {
            ...
            if ($now - $postDate < $twoDays) {
                ...
                echo '<tr>';
                    ...
                    echo '<td>';
                    echo str_replace(" ", "T", $post->post_modified_gmt)."Z";
                    echo '</td>';
    ...
    foreach ($posts as $post) {
        if ($trk_news_active && $trk_permission != 2) {
            ...
            if (1) { //if ($now - $postDate < $twoDays) {
                ...
                <url>
                    <loc>".trk_escapexml(get_permalink($post->ID))."</loc>
                    <lastmod>".str_replace(" ", "T", $post->post_modified_gmt)."Z"."</lastmod>
                </url>";
            }
        }
    }
    ...
    if($msg == ''){
        ...
        $xml_sitemap_google_news .= "
            <url>
                ...
                <lastmod>".str_replace(" ", "T", date('Y-m-d H:s:i'))."Z"."</lastmod>
            </url>
...
function XmlGenerator(){
                    ...
                    <td>
                        <?php 
                            if(file_exists(ABSPATH.'sitemap-post-page-'.$i.'.xml')){            
                                echo date("Y-m-d H:i:s ", filemtime(ABSPATH.'sitemap-post-page-'.$i.'.xml'))."(".wp_timezone_string().")";
                            }
                            else{
                                echo 'No created yet';
                            }
                    ...
                    <td>
                        <?php 
                            if(file_exists(ABSPATH.'sitemap-post.xml')){            
                                echo date("Y-m-d H:i:s ", filemtime(ABSPATH.'sitemap-post.xml'))."(".wp_timezone_string().")";
                            }
                            else{
                                echo 'No created yet';
                            }
                    ...
                    <td>
                        <?php 
                            if(file_exists(ABSPATH.'sitemap-page.xml')){            
                                echo date("Y-m-d H:i:s ", filemtime(ABSPATH.'sitemap-page.xml'))."(".wp_timezone_string().")";
                            }
                            else{
                                echo 'No created yet';
                            }
                    ...
                    <td>
                        <?php 
                            if(file_exists(ABSPATH.'sitemap-news.xml')){            
                                echo date("Y-m-d H:i:s ", filemtime(ABSPATH.'sitemap-news.xml'))."(".wp_timezone_string().")";
                            }
                            else{
                                echo 'No created yet';
                            }
                    ...
                    <td>
                        <?php 
                            if(file_exists(ABSPATH.'sitemap-category.xml')){            
                                echo date("Y-m-d H:i:s ", filemtime(ABSPATH.'sitemap-category.xml'))."(".wp_timezone_string().")";
                            }
                            else{
                                echo 'No created yet';
                            }
    ...
    /* ---Old code---
    if ($subQstring[0] == 'xml-generate' ) {
        //--get query string
        if(isset($subQstring[1]) && str_contains($subQstring[1], 'action=google_new')){
            if (str_contains($subQstring[1], 'action=auto')) { 
                ...
                echo $msg = "The xml file is generated with the name of sitemap-news.xml (<a target='_blank' href='".get_site_url()."/sitemap-news.xml'>".get_site_url()."/sitemap-news.xml</a>)";

            }
            
        }
        if(isset($subQstring[1]) && str_contains($subQstring[1], 'action=post')){

        }
        if(isset($subQstring[1]) && str_contains($subQstring[1], 'action=page')){

        }
        if(isset($subQstring[1]) && str_contains($subQstring[1], 'action=website')){
            ...
            echo "Please login to WP-Admin and goto: setting->xml generator setting";
            
            // Uncomment below code if you want to allow setting of xml generator from outside of admin
            ...
            //     echo "<center>Updated and xml generated</center>";
            // }
            // $trk_news_active = get_option('trk_news_active');
            // $trk_path = get_option('trk_path');
            // $trk_n_name = get_option('trk_n_name');
